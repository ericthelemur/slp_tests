#!/bin/bash
#
#SBATCH --job-name=slp-ri_count  # Job name for tracking
...
export label="counter-f$freq"
# export val=0.0
# for val in 10 20 30 50 75 100
# do
echo "STARTING $val"

function proj_conf_repl() {
    sed -i "s/#define OFF_TIMER_THRESHOLD .\+/#define OFF_TIMER_THRESHOLD $val/" "$1"
    echo "Replacing with $val"
}
export -f proj_conf_repl

# grep --color=never square2/ex0 "square7/"
find gen -type f -name "*.csc" -printf '%P\n' | ./scripts/run_all_tests.sh $freq RANDINIT_COUNTER "$label_$val"
# find gen -type f -name "*.csc" -printf '%P\n' | ./scripts/run_all_tests.sh --resume $dir $freq
echo "DONE WITH $val"
sleep 90
# done