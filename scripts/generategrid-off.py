...
# python3 scripts/generategrid-off.py -t square5 -r 5 -n 25
...
for file_no in range(args.init_numb, args.init_numb + args.number):
    broad_id = random.randrange(1, (args.radius+2) ** 2)
    nodes = ""

    # Write nodes
    id = 1
    for i in range(-1, args.radius+1):
        for j in range(-1, args.radius+1):
...
with open(os.path.join("gen", args.target_folder, "testscript.js"), "w") as opfile:
    with open(args.script_template) as infile:
        opfile.write(infile.read()
                        .replace("{{RADIUS}}", str(args.radius))
        )