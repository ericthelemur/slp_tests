...
parser.add_argument("--just-bars", "-b", action='store_true')
...
# cap_time_mean, cap_time_med = plot_box(ax[0, 1], found_time, "Time until Capture")
# moves_mean, moves_med = plot_box(ax[0, 2], moves, "Hunter Path Length until Capture")
# bc_mean, bc_med = plot_box(ax[1, 1], broadcasts, "Source Broadcasts")
# rec_mean, rec_med = plot_box(ax[1, 2], recieved, "Sink Recieves")

cap_rat = {k: float(c)/total[k] if total[k] else 0 for k, c in found.items()}
cap_rat = plot_bar(ax[0], cap_rat, "Capture Ratio")

del_rat = {k: 0 if not sum(broadcasts[k]) else (float(sum(recieved[k]))/sum(broadcasts[k])) for k in recieved.keys()}
del_rat = plot_bar(ax[1], del_rat, "Delivery Ratio")
    ...
    if args.safety_periods_file is None:
        with open(f"{args.folder}/{args.n}/avgs.json", "w") as f:
            ...
            data = {"capture_time_mean": cap_time_mean, "capture_time_median": cap_time_med, "capture_ratio": cap_rat, "delivery_ratio": del_rat, "resulting_safety_periods": sps,
                    "st_mean": st_mean, "et_mean": et_mean, "moves_mean": moves_mean, "bc_mean": bc_mean, "rec_mean": rec_mean
            }
            f.write(dumps(data, indent=4))
        plt.savefig(f"{args.folder}/{args.n}/graphs.png")
    else:
        ...
        with open(f"{args.folder}/{args.n}/sf.json", "w") as f:
            ...
            f.write(dumps(data, indent=4))
        plt.savefig(f"{args.folder}/{args.n}/graphssf.png")
